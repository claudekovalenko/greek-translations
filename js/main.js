...
import { loadState, saveState, clearState, wordKey, uid, scoreSummary } from './state.js';
...
function renderWord(p, v, w, wi) {
  ...
  const title = d ? `${w.lemma} — ${describeParse(w)}` : posLabel(w);
...
function renderDrill(panel, ctx) {
  ...
  panel.innerHTML = `
    <div class="drill-head">
      <div class="drill-word">
        ...
        <span class="lemma" data-lemma>lemma <span class="gk" lang="grc" data-lemma-text hidden>${esc(word.lemma)}</span><span class="gk" data-lemma-mask aria-hidden="true">· · ·</span><button class="btn btn-quiet btn-sm" type="button" data-reveal-lemma>show</button></span>
      </div>
  ...
  panel.querySelector('[data-reveal-lemma]').addEventListener('click', (e) => {
    const t = panel.querySelector('[data-lemma-text]');
    t.hidden = !t.hidden;
    panel.querySelector('[data-lemma-mask]').hidden = !t.hidden;
    e.target.textContent = t.hidden ? 'show' : 'hide';
  });
